...
int main(){
    ...
    else
    {
        ...
        int used[11] = {0}; // used[row] = 0, if the symbol with index row was not used yet used[row] = 1, if was used.
        int count = 0, new = 0, row = 0, column = 0;
        while(count < lenOfarray)
        {
            ...
            if (used[row] == 0)
            {
                ...
                if (column % key == 0)
                {
                    //column-index of the number. If it %k==0 row->used 
                    new = new*10 + arr[row];
                    count ++;
                    used[row] = 1;// is used
                }
